...
import utils
import config
from data import Feeder, align2d, Dataset
...
class Evaluator(Feeder):
    def __init__(self):
        super(Evaluator, self).__init__(Dataset())
    ...
    def create_feed(self, answer, question):
        ...
        qv, av = self.label_qa(question)
        return aids, qv, av, 1.0


    def evaluate(self, sess, model, answer, question):
        ...
        aids = sorted(enumerate(answer_logit[0]), key=lambda x:-x[1])[:10]
        qw = self.qids_to_sent([id for id,_ in qids])
        aw = self.aids_to_sent([id for id,_ in aids])
        print('==================================================')
        print('question', question)
        print(qw, aw)
        print('question score', [v for _,v in qids])
        print('answer score', [v for _,v in aids])
        return qw, aw 
