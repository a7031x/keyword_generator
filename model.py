...
class Model(object):
    def __init__(self, qi2c, ckpt_folder, train_mode=True, name='model'):
        ...
        self.question_word_weight[config.EOS_ID] = 0.2
    ...
    def create_seq_loss(self):
        with tf.name_scope('seq_loss'):
            ...
            neg_weight = self.question_word_weight.copy()
            neg_weight[config.EOS_ID] = 5
            crossent = func.sparse_cross_entropy(
                logit, self.input_target_question,
                neg_weight=neg_weight,
                pos_weight=self.question_word_weight) * mask
